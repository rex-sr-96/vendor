...
export const StaffManagementScreen: React.FC = () => {
  ...
  const openEditModal = (staff: StaffMember) => {
    ...
    setEditRole((staff?.role as RoleType) || 'Cashier');
  ...
  const getRoleBadge = (r: string) => {
    switch (r) {
      ...
      default:
        return 'bg-[#F59E0B]/10 text-[#B87C0D] border-[#F59E0B]/20';
    }
  };
    ...
    <div className="pb-20 pt-2 w-full space-y-6 select-none relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-[#E5E7EB]/70">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-[24px] font-black text-[#021526] tracking-tight">Staff & Roles</h1>
        ...
        <button
          ...
          className="h-10 px-4 rounded-xl bg-[#021526] hover:bg-[#061D33] text-white font-extrabold text-[13px] flex items-center justify-center gap-2 shadow-xs active-press cursor-pointer transition-colors self-start sm:self-auto"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
        <div className="bg-white rounded-2xl p-4 border border-[#E5E7EB] shadow-2xs">
          <p className="text-[11px] font-bold uppercase tracking-wider text-[#5F6368]">Total Staff</p>
          <p className="text-[22px] font-black text-[#021526] mt-1">{staffMembers.length}</p>
          <p className="text-[11px] font-semibold text-[#16A34A] mt-0.5">All accounts registered</p>
        </div>
        <div className="bg-white rounded-2xl p-4 border border-[#E5E7EB] shadow-2xs">
          <p className="text-[11px] font-bold uppercase tracking-wider text-[#5F6368]">Active On Ground</p>
          <p className="text-[22px] font-black text-[#16A34A] mt-1">{activeStaffCount}</p>
          <p className="text-[11px] font-semibold text-[#5F6368] mt-0.5">Enabled for venue duty</p>
        </div>
        <div className="bg-white rounded-2xl p-4 border border-[#E5E7EB] shadow-2xs">
          <p className="text-[11px] font-bold uppercase tracking-wider text-[#5F6368]">Managers</p>
          <p className="text-[22px] font-black text-[#F94001] mt-1">
            {staffMembers.filter((s) => s.role === 'Manager').length}
          </p>
          <p className="text-[11px] font-semibold text-[#5F6368] mt-0.5">Lead operations</p>
        </div>
        <div className="bg-white rounded-2xl p-4 border border-[#E5E7EB] shadow-2xs">
          <p className="text-[11px] font-bold uppercase tracking-wider text-[#5F6368]">Cashiers & Ops</p>
          <p className="text-[22px] font-black text-[#021526] mt-1">
            {staffMembers.filter((s) => s.role !== 'Manager').length}
          </p>
          <p className="text-[11px] font-semibold text-[#5F6368] mt-0.5">Ground & billing staff</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {staffMembers.map((staff) => (
          <div
            key={staff.id}
            className="bg-white rounded-2xl p-4 border border-[#E5E7EB] shadow-2xs hover:shadow-xs transition-all flex flex-col justify-between space-y-4"
          >
            <div>
              {/* Header: Avatar, Name, Role Badge */}
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 rounded-xl bg-[#021526] text-white flex items-center justify-center font-black text-[15px] shrink-0">
                    {staff.name.slice(0, 2).toUpperCase()}
                  </div>
                  <div>
                    <h3 className="text-[15px] font-black text-[#021526] tracking-tight">{staff.name}</h3>
                    <span
                ...
                {/* Status Badge */}
                <span
                  className={`px-2 py-0.5 rounded-full text-[10.5px] font-bold ${staff.status === 'Active'
              ...
              {/* Contact Details */}
              <div className="mt-3.5 p-3 rounded-xl bg-[#F3F4F4] border border-[#E5E7EB] space-y-1.5 text-[12px] font-medium text-[#5F6368]">
                <div className="flex items-center gap-2">
                  <Phone className="w-3.5 h-3.5 text-[#F94001]" />
                  <span className="font-bold text-[#021526]">{staff.phone || (staff as any).phone_number || 'Not provided'}</span>
                </div>
                <div className="flex items-center gap-2">
            ...
            <div className="pt-3 border-t border-[#F3F4F4] flex items-center justify-between gap-2">
              <button
                type="button"
                onClick={() => openEditModal(staff)}
                className="px-3 py-1.5 rounded-xl bg-[#F3F4F4] hover:bg-[#F3F4F4] border border-[#E5E7EB] text-[12px] font-bold text-[#021526] flex items-center gap-1.5 cursor-pointer transition-colors"
              >
                <Edit2 className="w-3.5 h-3.5 text-[#F94001]" />
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-[11.5px] font-bold cursor-pointer transition-colors ${staff.status === 'Active'
                    ? 'bg-[#DC2626]/10 hover:bg-[#DC2626]/20 text-[#DC2626]'
                    : 'bg-[#16A34A]/10 hover:bg-[#16A34A]/20 text-[#15803D]'
                  }`}
              >
            ...
            >
              <div className="flex items-center justify-between pb-2 border-b border-[#F3F4F4]">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-[15px] font-black text-[#021526]">Add Staff Member</h3>
                    <p className="text-[11px] text-[#5F6368]">Select normal operational role</p>
                  </div>
                </div>
              ...
              <form onSubmit={handleAddSubmit} className="space-y-3">
                <div>
                  <label className="block text-[11px] font-bold text-[#5F6368] mb-1">
                    Full Name <span className="text-[#F94001]">*</span>
                  </label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-[11px] font-bold text-[#5F6368] mb-1">
                    Phone Number <span className="text-[#F94001]">*</span>
                  </label>
                  <input
                    type="tel"
                    required
                ...
                <div>
                  <label className="block text-[11px] font-bold text-[#5F6368] mb-1">Email Address</label>
                  <input
                    type="email"
                ...
                <div>
                  <label className="block text-[11px] font-bold text-[#5F6368] mb-1.5">
                    Assign Normal Role <span className="text-[#F94001]">*</span>
                  </label>
                    ...
                    {AVAILABLE_ROLES.map((r) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className={`text-[10px] block mt-0.5 ${isSelected ? 'text-[#021526]/70 font-semibold' : 'text-[#5F6368]'}`}>
                            {r.desc}
                          </span>
                        </button>
                      );
        ...
        {editingStaff && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs select-none">
            ...
            >
              <div className="flex items-center justify-between pb-2 border-b border-[#F3F4F4]">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-xl bg-[#021526] text-white flex items-center justify-center">
                    <Edit2 className="w-4 h-4 text-[#F94001]" />
                  </div>
                  <div>
                    <h3 className="text-[15px] font-black text-[#021526]">Edit Staff & Role</h3>
                    <p className="text-[11px] text-[#5F6368]">Update name, contact & assigned role</p>
                  </div>
                </div>
              ...
              <form onSubmit={handleEditSubmit} className="space-y-3">
                <div>
                  <label className="block text-[11px] font-bold text-[#5F6368] mb-1">
                    Staff Name <span className="text-[#F94001]">*</span>
                  </label>
                  <input
                    type="text"
                    required
                ...
                {/* Normal Role Selector */}
                <div>
                  <label className="block text-[11px] font-bold text-[#5F6368] mb-1.5">Assigned Normal Role</label>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {AVAILABLE_ROLES.map((r) => {
                ...
                <div className="pt-2">
                  <button
                    type="submit"
                    className="w-full h-10 rounded-xl bg-[#021526] hover:bg-[#061D33] text-white font-black text-[13px] flex items-center justify-center gap-1.5 shadow-sm active-press cursor-pointer transition-colors"
                  >
